...
def training(X, y, X_test, y_test):
    ...
    # ])
    model = Sequential([
        Dense(256, input_dim=np.shape(X)[1]),
        Activation('relu'),
        Dense(64, input_dim=np.shape(X)[1]),
        Activation('relu'),
        Dense(32, input_dim=np.shape(X)[1]),
        Activation('relu'),
        Dense(16, input_dim=np.shape(X)[1]),
        Activation('relu'),
        Dense(8, input_dim=np.shape(X)[1]),
        Activation('relu'),
        Dense(1)
    ])
    ...
    model.compile(optimizer=adam,
                  loss='mean_absolute_error',   # loss function  mean_squared_error; mean_absolute_percentage_error
                  metrics=[r2_keras])           # metrics for test in training
    # training
    record = model.fit(X, y, epochs=150, batch_size=64, verbose=2,
...
def NN_train(matfile, modelfile):
    ...
    #  Reshape Data
    sat_band = [412, 443, 488, 531, 547, 667]  # MODIS satellite bands
    ...
    output_save = pd.DataFrame(np.concatenate([np.array(y_test),
                                               y_test_pre], axis=1))
    output_save.to_excel('Val_test_results_org6b.xlsx')
...
def NN_val(matfile, modelfile):
    ...
    output_save.to_excel('Val_results6.xlsx')
...
def NN_val_IOCCG(modelfile):
    ...
    iRrs531 = Rrs531 / (1 + (LUT[3][1] * Rrs_ratio + LUT[3][2] * Rrs547 ** LUT[3][3]))
    iRrs547 = Rrs547 / (1 + (LUT[4][1] * Rrs_ratio + LUT[4][2] * Rrs547 ** LUT[4][3]))
    iRrs667 = Rrs667 / (1 + (LUT[5][1] * Rrs_ratio + LUT[5][2] * Rrs547 ** LUT[5][3]))

    X_norm =pd.concat([iRrs412, iRrs443, iRrs488, iRrs531, iRrs547, iRrs667], axis=1)  # raman corr
    ...
    output_save = pd.DataFrame(np.concatenate([np.array(y_Y),
                                              y_predict], axis=1))
    output_save.to_excel('Val_IOCCG_org_6b.xlsx')
...
def NN_val_field(modelfile):
    ...
    # load field data
    matData = sio.loadmat('fieldsum.mat')
    wavelength = matData['field']['wl'][0][0].squeeze()
    Rrs = pd.DataFrame(matData['field']['Rrs'][0][0], columns=wavelength)
    y_Y = pd.DataFrame(matData['field']['Y'][0][0], columns=['Y'])
    ...
    # X_norm = pd.concat([Rrs412, Rrs443, Rrs488, Rrs531, Rrs547, Rrs667], axis=1)
    # load model
    model_dir = './Model/'
    model = load_model(model_dir + modelfile, custom_objects={'r2_keras': r2_keras})
    y_predict = model.predict(X_norm)
    output_save = pd.DataFrame(np.concatenate([np.array(y_Y),
                                               y_predict], axis=1))
    output_save.to_excel('Val_fieldsum_org_6bM.xlsx')

    # draw_val_Y(y_Y, y_predict, saving=True, name='Val_field_Y')

def NN_val_MODIS(modelfile):

    # load field data
    matData = sio.loadmat('SPG_Rrs.mat')
    wavelength = matData['field']['wl'][0][0].squeeze()
    Rrs = pd.DataFrame(matData['field']['Rrs'][0][0], columns=wavelength)
    y_Y = pd.DataFrame(matData['field']['Y'][0][0], columns=['Y'])
    ...
    output_save = pd.DataFrame(np.concatenate([np.array(y_Y),
                                               y_predict], axis=1))
    # output_save.to_excel('Val_TPO_Rrs.xlsx')

    savemat('Val_SPG_Rrs_6b.mat', {'Y':y_predict})
    # draw_val_Y(y_Y, y_predict, saving=True, name='Val_field_Y')
